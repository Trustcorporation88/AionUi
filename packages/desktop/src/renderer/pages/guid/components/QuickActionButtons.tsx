...
import { Brain, Earth } from '@icon-park/react';
...
const QuickActionButtons: React.FC<QuickActionButtonsProps> = ({
  ...
  const [hoveredQuickAction, setHoveredQuickAction] = useState<'guide' | 'bugReport' | 'repo' | 'webui' | null>(null);
  ...
  return (
    <>
      ...
      >
        <div className='flex justify-center items-center gap-24px'>
          <div
            ...
            style={quickActionStyle(hoveredQuickAction === 'guide')}
            onMouseEnter={() => setHoveredQuickAction('guide')}
            ...
            <div className='relative w-20px h-20px flex-shrink-0 leading-none'>
              <div className='absolute inset-0 flex items-center justify-center'>
                <Brain
                  ...
                  size={20}
                  fill='currentColor'
                  className='block text-[var(--color-text-3)] group-hover:text-brand transition-colors duration-300'
                />
              </div>
            </div>
      ...
      <AgentGuideModal visible={guideVisible} onClose={() => setGuideVisible(false)} />
    </>
  );
};
